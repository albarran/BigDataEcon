...
- Entre otras, tu análisis debería abordar las siguientes cuestiones:
  ...
  3. ¿Qué características están relacionadas con el riesgo de las startups? Pista: considera al menos `ltv_cac_ratio`, `burn_rate`, `runway_meses` y `tasa_retencion`.
  
<!--  4. Relación entre `tasa_retencion` y probabilidad de éxito. -->

  4. Comparar indicadores clave (financiación, valoración, probabilidad de éxito) entre los principales países (los cinco países con más empresas).
  
  5. Patrones de madurez: ¿cómo cambian (o no) las características de las startups con la `etapa`?
