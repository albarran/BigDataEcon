...
```{r}
lm(data=descuento, log(ventas) ~ renta + I(renta^2) + descuento) %>% summary()
```
...
```{r}
lm(data=descuento , ventas ~ cut_width(renta, 500) + descuento) %>% summary()
```
...
```{r}
lm(data=descuento, ventas ~ renta*descuento)  %>% summary()
```

* Principio jerárquico: al incluir una interacción *siempre* deben incluirse los factores principales (NO sólo `renta:descuento`)
