...
macro <- data.frame(
  ...
  norte = c("No", "Sí", "Sí", "No", "No", "No", 
            "Sí", "Sí", "Sí", "Sí", "Sí", "Sí"),
  tipo = c(rep("Desarrollada", 12))
) %>%
  mutate(across(c(norte, tipo), as.factor))
...
macro_info <- data.frame(
  variable = c("pais", "pib", "desempleo", "inflacion", "deuda", "norte", "tipo"),
  descripcion = c(
    "Nombre del país",
    ...
    "Clasificación económica del país"
  ),
  tipo = c("Carácter", "Numérica", "Numérica", "Numérica", "Numérica", "Factor", "Factor")
)
...
|------|---------|---------------|
| **Puntos** | `geom_point()` | Dispersión, correlaciones |
| **Líneas** | `geom_line()` | Series temporales |
...
| **Suavizado** | `geom_smooth()` | Tendencias, regresión |
| **Texto** | `geom_text()` | Etiquetas, anotaciones |
...
ggplot(ventas, aes(x = marketing, y = ventas)) +
  geom_point(aes(color = region, size = satisfaccion))
```
...
ggplot(macro, aes(y = desempleo, x = pib)) +
  geom_point(aes(shape = norte, color = deuda))
```
...
- **Principio**: Añadir señal visual para representar información adicional, NO para "embellecer"
...
#| eval: false
ggplot(clientes, aes(x = ingresos, y = gasto)) + geom_point()
```

- Pero otros gráficos representan estadísticas calculadas a partir de los datos

- `geom_smooth()`: regresión → predicciones
```{r smoothers}
#| eval: false
...
- Para variables continuas, no existen categorías "naturales" para calcular las frecuencias

  - Se tienen que definir "arbitrariamente" los intervalos
  
  - Distintas elecciones pueden revelar distinta información
...
- Nota: es redundante asociar dos estéticas a una variable

## Escala Logarítmica

- En muchos contextos, las variables tienen un **rango de valores amplio**
  
  - PIB (Malta €16B vs EE.UU. €26T), clientes (pyme 500 vs Inditex 200M)
...
- Usar logaritmos cambia interpretación de diferencias absolutas a porcentuales

  - Más relevante +10% <!--clientes--> o +500 clientes (mucho en pyme, nada en Inditex)

-  Los logs NO solucionan problemas de atípicos ni su visualización
...
- ¿Qué diferencia hay con `x = log(pib)`?
...
## Personalizar el aspecto general: Color
...
- Es preferible usar **paletas predefinidas** con criterios de diseño y visualización de información:
...
## Personalizar el aspecto general: Temas
