...
* **¿Hay un menú para poner esto sin teclearlo?** En **RStudio** sí: cada celda lleva una rueda dentada que abre un diálogo con las opciones más comunes. En **Positron**, las opciones se escriben a mano

    + también en el modo visual: el bloque se ve con su formato, pero las opciones siguen siendo texto dentro de la celda

    + no es un problema: son dos líneas y se escriben antes de darse cuenta

<!-- INTERNO: comprobado el 15-sep en la extension Quarto 1.137.0 instalada en Positron. No hay
ningun comando ni icono para opciones de celda: el unico $(gear) de la extension es el del
dialogo de crear proyecto. Si alguien ve una rueda en Positron, sera la del editor de NOTEBOOKS
(.ipynb), que es otra cosa. Verificar de nuevo si cambia la version de la extension. -->
