...
* Se guarda con el **disquete** y se renderiza con **Preview**; el resultado sale en el **visor** de la derecha, y el fichero `.html` aparece en el **Explorador** (doble clic para abrirlo)

![](figure/positron-quarto-guia.png){width="78%" fig-align="center"}

::: {style="font-size: 70%;"}
También con atajos (`Ctrl+S`, `Ctrl+Shift+K`) o desde la **Terminal**: `quarto render informe.qmd`
:::
...
* Se cambia de uno a otro con el icono **⇄** de la barra del editor (arriba a la derecha), con `Ctrl+Shift+F4`, o desde la paleta (`Ctrl+Shift+P`) con *Quarto: Edit in Visual Mode*

    + la primera vez avisa de que el fichero se reescribirá con el formato de Pandoc: aceptar

* Al pasar a modo visual aparece esta **barra de herramientas**:
...
:::: {.columns}
::: {.column width="50%"}
![](figure/positron-quarto-insert.png){width="98%"}
:::
::: {.column width="50%"}
![](figure/positron-quarto-insert-cell.png){width="98%"}
:::
::::

* En **modo source** se escribe a mano: entre ```` ```{r} ```` y ```` ``` ````; y el código **en medio del texto**, con `` `r ` `` y el código dentro
...
- Otras que salen en la slide siguiente: `output: false` oculta solo el resultado, `include: false` no muestra nada pero SÍ ejecuta, y `message`/`warning`/`error` controlan lo que R va contando.
...
::: {.notes}
- Para ir más lejos: `broom::tidy()` convierte resultados de modelos en tablas; `modelsummary` genera tablas de regresión y de descriptivos; también existen `printr`, `pander`, `stargazer` y `xtable`.
