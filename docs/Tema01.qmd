...
![](figure/positron-quarto-insert-cell.png){width="62%" fig-align="center"}

* Para **código dentro del texto** (que el resultado salga en medio de una frase): *Insert > Any...* (`Ctrl+/`) y escribir `inline`, o el icono `</>` de la barra

* En **modo source**, lo mismo se escribe a mano: el fragmento entre ```` ```{r} ```` y ```` ``` ````; el código en línea, con `` `r ` `` y el código dentro
