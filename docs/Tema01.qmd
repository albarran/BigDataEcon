...
-   Instalar Quarto para vuestro sistema operativo desde [aquí](https://quarto.org/docs/get-started/). La web pide después elegir un editor: **en Positron no hay que hacer nada más**, porque ya trae la extensión de Quarto incorporada
...
* Creamos un nuevo documento en *File > New File...* eligiendo *Quarto Document*, y lo guardamos con extensión `.qmd`

* Se renderiza con el botón **Preview**, arriba a la derecha del editor

![](figure/positron-quarto-preview.png){width="62%" fig-align="center"}

* El resultado aparece **al lado, en el visor** (el panel de la derecha)

* Y en el **Explorador** (barra de la izquierda) aparece un **archivo nuevo** en la carpeta: el `.html` generado. Doble clic sobre él para abrirlo

::: {.notes}
- Señalar con el ratón las TRES cosas: el botón Preview, el panel del visor y el fichero nuevo en el Explorador. Que lo vean en pantalla, no en la slide.
- Al doble clic sobre el .html, Positron lo abre en su propio visor.
- Atajos y Terminal: NO insistir, van a usar los botones. Están en las notas por si alguien pregunta (`Ctrl+Shift+K` para Preview; `quarto render fichero.qmd` en la pestaña Terminal, junto a la consola).
:::
...
* Para cambiar de uno a otro: botón derecho sobre el documento y *Edit in Visual Mode*, o `Ctrl+Shift+F4`

* Al pasar a modo visual aparece arriba una **barra de herramientas**, con accesos a
...
::: {.notes}
- SEÑALAR en pantalla: dónde está la barra, y que al volver a source el texto está ahí escrito. Es lo que convence de que no son dos documentos distintos.
- Comprobado en la extensión Quarto 1.137.0: el comando es `quarto.editInVisualMode` y el atajo `Ctrl+Shift+F4`.
...
"code chunks"
::::
* En **modo visual**, con el botón **Insert** de la barra de herramientas:

    + *Code Cell > R* inserta un **fragmento** (varias líneas de código)
    + *Inline Code* inserta **código dentro del texto**, para que el resultado salga en medio de la frase

. . .

* En **modo source**, lo mismo se escribe a mano:

    + el fragmento, entre ```` ```{r} ```` y ```` ``` ````
    + el código en línea, con `` `r ` `` y el código dentro

* Lo que aparece en el documento (el código, su resultado, los dos o ninguno) se controla con **opciones**: en una celda concreta, o para todo el documento desde la cabecera
...
- El código en línea es el que más les sorprende: una frase con un número que se recalcula solo.
:::
...
::: {.notes}
- Las opciones se escriben SIEMPRE a mano, también en modo visual: no hay menú para ellas en
  Positron (comprobado en la extensión Quarto 1.137.0; RStudio sí tiene una rueda por celda).
  No merece una slide: si alguien pregunta, se contesta y se sigue.
- La tabla es exactamente lo que pide el apartado b.ii del ejercicio.
- El error de todos los años: escribir `#` en vez de `#|`. Conviene provocarlo en vivo.
:::
