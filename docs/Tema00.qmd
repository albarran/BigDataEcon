...
format:
  revealjs:
    ...
    # height: 2000
    # margin: 0.05
  # FORMATO beamer RETIRADO el 15-sep-2026 (Pedro: nadie ha pedido el PDF, y el render
  # del sitio completo lo generaba solo, publicandolo en las tres webs). Para recuperarlo,
...
![](figure/positron-gui.png){width="58%" fig-align="center"}
...
## Proyectos: siempre una carpeta, a veces con ajustes

* Todo el trabajo sucede **dentro de un directorio**. Eso ES el proyecto, y con eso basta: abrir una carpeta ya es decir "este es mi directorio de trabajo, aquí organizo mis datos, mi código y mis resultados"

* Lo demás que oiréis nombrar son **la misma carpeta con un fichero de ajustes añadido**:

::: {.smaller-output}

| | qué es | para qué |
|---|---|---|
| carpeta abierta | *File \> Open Folder* | lo que usamos en el curso |
| **proyecto de R** | la misma carpeta + un fichero **`.Rproj`** | guarda ajustes de R que queréis conservar para ese proyecto, y al abrirlo fija el directorio de trabajo |
| **proyecto de Quarto** | la misma carpeta + un fichero **`_quarto.yml`** | ajustes comunes a **muchos** documentos de Quarto, para no repetirlos en cada uno |

:::

* Para dos o tres ejercicios sueltos no hace falta ninguna de las dos cosas

::: {.notes}
- La idea que tienen que llevarse es que NO son tres cosas distintas: lo comun es todo (una
  carpeta que contiene el trabajo) y lo que cambia es un fichero de ajustes. Presentarlo como
  tres conceptos separados los lia, y ademas es falso.
- Verificado en la documentacion de Posit (24-sep-2026): un proyecto es "un contenedor que
  organiza el trabajo"; el `.Rproj` guarda las opciones del proyecto y al abrirlo R arranca una
  sesion nueva y fija el directorio de trabajo a esa carpeta.
- Verificado en Positron: las plantillas de *File > New Folder* son Python Project, R Project,
  Jupyter Notebook y Empty Project.
- La web de esta asignatura SI es un proyecto de Quarto: decenas de paginas que comparten
  formato y se publican juntas. Sirve de ejemplo si preguntan para que vale.
:::

## Ficheros que R crea solo

* Trabajando con R aparecerán en vuestra carpeta ficheros que no habéis creado vosotros:

  - **`.RData`**: el **espacio de trabajo** guardado, es decir, los objetos que había en memoria
  - **`.Rhistory`**: el **historial** de órdenes que habéis escrito en la consola
  - **`.Rproj.user`**: una carpeta oculta con ficheros temporales, si usáis un proyecto de R

* Los tres los genera R por su cuenta: son **estado de vuestra sesión**, no vuestro trabajo. No se entregan y no hace falta tocarlos

::: {.notes}
- Por eso un proyecto de R los mete en su `.gitignore` nada mas crearlo (comprobado en el
  binario de Positron: `.Rproj.user`, `.Rhistory`, `.RData`, `.Ruserdata`).
- Conviene decirlo aqui porque los van a ver aparecer y se preguntan que son; y de paso enlaza
  con la idea de que el trabajo es el .qmd y los datos, no lo que R deja por el camino.
:::
...
* Lo que se entrega es el trabajo de las **PRÁCTICAS** de este tema (el resto de cosas que
  probéis o veamos en pantalla, no)

* Para este tema, ese trabajo va en un guion propio: `Tema00_ID.R` (p.e.,
  `Tema00_12345678.R`), en la carpeta del curso

    + con **DNI**, los 8 números quitando la letra del final; con **NIE**, la letra del
      principio y los números, quitando la del final; con **pasaporte** u otro documento,
      entero, sin quitar nada

    + cada tema tendrá su propia entrega (y su formato); en algunos temas podrá haber más de una

* Si una tarea pide *contestar* algo (no solo código), escribid la respuesta como TEXTO en
  un comentario del guion
...
::: {.notes}
Formulario de "trabajo de clase del tema" (TemaNN_clase), distinto del de ejercicio (TemaNNej).
El enlace/QR se proyecta solo cuando hay recogida.
:::
...
```{r}
miLista <- list(
  saludo = "hola", vec = x, lista = list(1:4, "X"),
  datos = datos
)
```
...
-   Muchas funciones de R devuelven sus **resultados como una lista**: guardan cosas diversas
    (estadísticos, opciones, datos...) en un único objeto

-   Por eso importa saber ACCEDER a sus elementos: cuando ejecutemos un contraste o una
    regresión, extraeremos de ahí lo que necesitemos
...
#| echo: false
nombres <- c(
  "area", "temperatura", "tamaño", "almacenaje", "método",
...
```{r}
#| echo: false
perritos <- read_delim("data/hotdogs.txt",
  delim = "\t",
...
export(datos, "datos/fichas.csv"); datos2 <- import("datos/fichas.csv"); str(datos2)
  # el factor vuelve como character/entero: el csv no guarda tipos -> enlaza "el tipo importa"
Práctica creada 9-sep: guardar/cargar/importar DENTRO del running example (antes solo
estaba en el ejercicio de reserva), y COLOCADA tras las slides de save/load y rio
(regla: nunca pedir en práctica lo aún no visto).
-->
