...
- En este ejercicio vamos a practicar los conceptos básicos de R con datos NUEVOS, distintos de los de clase: el objetivo es comprobar que sabéis aplicarlos sin el guion delante. Debéis escribir un archivo de código de R con los comandos necesarios para responder a los siguientes ejercicios. Podéis encontrar una plantilla para vuestras respuestas [aquí](Tema00ej1_123456789.R).

- Recordad la regla con la IA: puede ayudaros a *entender* (un error, una función); si os escribe la solución y la copiáis sin entenderla, el ejercicio no os habrá servido y se notará después.
...
NOTA: se recomienda crear una carpeta para el ejercicio, abrirla en Positron (*File \> Open Folder*) y guardar el archivo `Ventas1.xlsx` en ella.
...
## mismos pasos que en Excel

## cargar datos
library(rio)
datos <- import("Ventas1.xlsx")
# Ventas1Lunes <- Ventas1$Lunes

# Extraer ventas de cada tienda
# Elegir los "objetos"/celdas apropiados
...
a) Los archivos [`Ventas2a.xlsx`](https://raw.githubusercontent.com/albarran/00datos/main/Ventas2a.xlsx) y [`Ventas2b.xlsx`](https://raw.githubusercontent.com/albarran/00datos/main/Ventas2b.xlsx) contiene el valor de las ventas de tres productos cada uno, a nivel nacional y a nivel internacional (en millones de euros). Combinar ambos conjuntos de datos en uno solo con las información de todos los productos.
    ...
    * PISTA: `rbind()` añade filas y `cbind()` columnas a un *data frame* (consultad la ayuda: `?rbind`).
...
precios <- data.frame(
  "PrecioN" = c(5.2, 4.7, 5.7, 6.1, 7, 6.7),
...
factor_satisf_vector <- factor(satisf_vector,
  ordered = TRUE,
...
b) Generar un vector, `sum_factor`, con las estadísticas descriptivas de la información del Problema anterior y muestra el resultado ("imprime" ese vector). 
...
a) El conjunto de datos `mtcars` está incluido en R por defecto; buscar `mtcars` en la ayuda (`?mtcars` o el panel Help de Positron) para conocer las variables que incluye. Comprobar la estructura con `str()` y visualizar las primeras observaciones con `head()`.
...
Rellenad este [FORMULARIO](https://forms.gle/Miz3FVYq1pUa8VPY6) con vuestros datos y subid

-   vuestro archivo de R

<!--  * el fichero .csv que utilizáis.. -->

IMPORTANTE: el nombre de los ficheros que subáis DEBE seguir este formato, que incluye vuestro identificador: ej.,

-   Tema00ej1_12345678.R

Con **DNI**, los 8 números quitando la letra del final. Con **NIE**, la letra del principio y los números, quitando la del final. Con **pasaporte** u otro documento, entero, sin quitar nada.
