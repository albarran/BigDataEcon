...
# ========================================
# PRODUCTOS (sin tabla de categorías)
# ========================================
productos <- data.frame(
  id_producto = 1:150,
  nombre_producto = paste0("Producto_", sprintf("%03d", 1:150)),
  categoria = sample(c("Electrónica", "Ropa", "Hogar", "Deportes", 
...
tiendas <- data.frame(
  id_tienda = 1:12,
  codigo_tienda = paste0("T", sprintf("%03d", 1:12)),
...
# ========================================
# COMISIONES_EMPLEADO 
# CLAVE COMPUESTA: codigo_tienda + puesto
# (nombre diferente: codigo_tienda en vez de id_tienda)
# ========================================
comisiones_empleado <- expand.grid(
  codigo_tienda = paste0("T", sprintf("%03d", 1:12)),
  puesto = c("Vendedor", "Supervisor", "Gerente", "Almacén")
) %>%
  mutate(
    comision_porcentaje = case_when(
      puesto == "Vendedor" & codigo_tienda %in% c("T001", "T002") ~ 3.0,
      puesto == "Vendedor" ~ 2.5,
      puesto == "Supervisor" ~ 1.5,
      puesto == "Gerente" ~ 1.0,
      TRUE ~ 0.5
    )
  )
...
productos_info <- data.frame(
  variable = c("id_producto", "nombre_producto", "categoria", "precio", "costo", "activo"),
  descripcion = c(
  ...
  tipo = c("Entero", "Carácter", "Carácter", "Numérica", "Numérica", "Lógico")
)
...
tiendas_info <- data.frame(
  variable = c("id_tienda", "codigo_tienda", "nombre_tienda", "region"),
  descripcion = c(
    "Identificador único de la tienda",
    "Código alfanumérico de la tienda (formato T001-T012)",
    "Nombre comercial de la tienda",
    "Región geográfica (Centro/Norte/Sur/Nordeste/Levante/Noroeste)"
  ),
  tipo = c("Entero", "Carácter", "Carácter", "Factor")
)

comisiones_empleado_info <- data.frame(
  variable = c("codigo_tienda", "puesto", "comision_porcentaje"),
  descripcion = c(
    "Código de la tienda",
    "Puesto del empleado",
    "Porcentaje de comisión por venta"
  ),
  tipo = c("Carácter", "Factor", "Numérica")
)
...
save(ventas, productos, clientes, empleados, tiendas, 
     comisiones_empleado, devoluciones, ventas_ancho,
     ventas_info, productos_info, clientes_info, empleados_info, 
     tiendas_info, comisiones_empleado_info, devoluciones_info,
