...
* Quitad ahora `group = 1` y mirad el resultado: las tres barras valen 1. ¿Proporción respecto a qué está calculando en cada caso?

## PRÁCTICA en clase (2): el gráfico calcula por ti (cont.)

3. Distribución del `gasto` con `geom_histogram()`, probando `bins = 10`, `bins = 40` y `binwidth = 250`. ¿Cuál de las tres versiones le enseñaríais a dirección y por qué?

4. Repetid las tres versiones con `ingresos`. ¿Hay un grupo de clientes de ingresos muy bajos? ¿Con cuál de las tres se ve y con cuáles no?

5. Añadid un `geom_boxplot()` del `gasto` por `segmento`

* En una frase: ¿qué significa que un `geom` "tiene una estadística por defecto"?
...
2) Proporciones reales: Económico 0,280, Estándar 0,497, Premium 0,223 (verificado en R).
   ...
   El `group = 1` es lo que hace falta explicar: sin el, cada barra es su PROPIO grupo y la
...
## PRÁCTICA en clase (3): la misma pregunta en dos escalas

Una consultora de inversión compara el perfil de ingresos de cinco grandes corporaciones y cinco startups. Copiad estos datos:
...
## PRÁCTICA en clase (3): la misma pregunta en dos escalas (cont.)
...
5. Un compañero dice que usa logaritmos "para quitar los valores extremos". ¿Está bien dicho? ¿Qué está haciendo en realidad la escala logarítmica?

* Poned título, ejes con unidades y color por tipo. El gráfico tiene que poder enseñarse solo, sin que vosotros lo expliquéis al lado.
...
::: {.callout-note}
**Ahora el ejercicio 1, apartado d)**: facetas por `canal`, `labs()` completo y probar temas, incluido uno de `ggthemes`.
:::
