...
* [Tipos principales](http://eitc.org/research-opportunities/photos6/different-machine-learning-models_010924a/image_large)
...
1. Usando **logaritmos**, modelizamos elasticiades (cambios porcentuales) y relaciones no lineales
...
2. Usando **polinomios** (y otras funciones no lineales), modelizamos relaciones no lineales cuyos efectos varían con el nivel de la variable
...
## Métricas de error de predicción (regresión)

::: {style="font-size: 95%;"}

* Un modelo es mejor si sus predicciones se ajustan mejor a las observaciones
...
* Una **función de pérdida (o coste)** evalúa cómo valoramos las desviaciones
  ...
  1. **Mean Square Error** (Error Cuadrático Medio): $\small \color{blue}{MSE(y,\widehat{y})={\frac{1}{n}\sum_{i=1}^{n}\left(y-\widehat{y}\right)^2}}$
      ...
      + $\small \color{blue}{R^2}$ y $\small \color{blue}{R^2-ajustado}$: variantes solo válidas para comparar modelos con la *misma variable dependiente*.
  ...
  * Otras medidas basadas en distintas funciones de pérdida, la verosimilitud del modelo ($\small \color{blue}{AIC}$, $\small \color{blue}{BIC}$), etc
